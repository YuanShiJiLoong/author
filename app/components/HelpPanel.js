...
| 设置项 | 说明 |
|------|------|
| **API 提供商** | 支持智谱、DeepSeek、OpenAI、Gemini、Kimi网络等，也可选自定义接入 |
...
### 存档 / 读档（项目级别）
在侧栏底部：
- **💾 存档** — 将整个项目（所有章节 + 设定集 + AI 对话历史）导出为 JSON 文件
- **📂 读档** — 从 JSON 文件恢复完整项目

> 建议定期存档，防止浏览器数据被清空。

### 章节导出
- **导出本章** — 将当前章节导出为 Markdown (.md) 文件
- **导出全部** — 将所有章节合并导出为一个 Markdown 文件

### 重要提醒
- 所有数据存储在 **浏览器本地**，不会上传到任何服务器
- **清除浏览器数据** 会丢失所有未导出的内容
- **API Key** 仅存储在本地 localStorage 中
- 建议定期使用「💾 存档」功能备份
    `,
    },
    {
